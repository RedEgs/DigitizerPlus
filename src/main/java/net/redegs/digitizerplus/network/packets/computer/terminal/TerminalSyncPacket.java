...
public class TerminalSyncPacket {
    ...
    private final int[] colors;   // rows*cols
    ...
    public TerminalSyncPacket(Cell[][] buffer, int cursorX, int cursorY) {
        ...
        this.colors = new int[rows * cols];
        int i = 0;
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < cols; x++) {
                Cell cell = buffer[y][x];
                this.chars[i] = cell.ch;
                this.colors[i] = cell.color;
                i++;
            }
        }
    ...
    private TerminalSyncPacket(int rows, int cols, char[] chars, int[] colors, int cursorX, int cursorY) {
        this.rows = rows;
        this.cols = cols;
        this.chars = chars;
        this.colors = colors;
    ...
    public static TerminalSyncPacket decode(FriendlyByteBuf buf) {
        ...
        return new TerminalSyncPacket(rows, cols, chars, colors, cursorX, cursorY);
    }
            ...
            if (context.getDirection().getReceptionSide().isClient()) {
                ...
                for (int y = 0; y < pkt.rows; y++) {
                    for (int x = 0; x < pkt.cols; x++) {
                        buffer[y][x] = new Cell(pkt.chars[i], pkt.colors[i]);
                        i++;
                    }
                }
